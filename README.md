...
A companion entity that lives in a directory on your own machine:
persistent memory, drives that wake it, per-person privacy walls, a
dark-sky web face you can install on your phone, and the ability to
reach you — a push notification when it has something genuinely worth
your pocket.

**Pure Python stdlib.** No pip dependencies (pytest only for the test
suite), no frameworks, no build step. It speaks to any
OpenAI-compatible endpoint; a local llama-server is the intended home.
The full design is in [ARCHITECTURE.md](ARCHITECTURE.md) — but you
...
The Observatory is a PWA (Phase 8a): a manifest, a service worker and
a maskable icon are generated by the entity itself — nothing to build,
nothing to download from a store.

**Android / desktop Chrome & Edge:**

1. Open `http://<server-lan-ip>:8762/` in the browser.
2. Accept the one-time *"keep it on your home screen?"* affordance, or
   use the browser menu → **Install app** / **Add to home screen**.
3. Launch from the icon — full-screen, dark-sky, standalone.

**iPhone / iPad (iOS 16.4+):**

1. iOS allows Web Push **only for PWAs installed to the home screen,
   served over HTTPS**. Plain-HTTP works fine for everything *except*
   push and home-screen install polish.
2. Run the entity with TLS:

   ```bash
   anima service install --root ~/entities/me --tls    # or: anima run ... --tls
   ```

   This generates a self-signed certificate under
   `<root>/identity/tls/` (via the system `openssl`) and serves
   `https://<server-lan-ip>:8762/`.
3. First visit: Safari will warn about the self-signed certificate —
   this is **trust-on-first-use**: inspect it once, accept it, and
   Safari remembers. (For a warning-free setup, put a real cert from
   your own CA or a reverse proxy in `identity/tls/cert.pem` +
   `key.pem` — the entity serves whatever lives there.)
4. Share button → **Add to Home Screen** → open from the icon.

**Let it reach you:** in the Observatory's *reach* card, tap *"let
<entity> reach you here"* and grant notification permission. The
subscription lands on **your** relationship record (multiple devices
stack; unsubscribe any time from the same card). From then on the
entity's `notify` tool — budget-railed, ledger-logged, and held to the
bar *"would a considerate friend send this text?"* — can tap you on
the shoulder even with the app closed.

**Plain-HTTP fallback:** no TLS, no problem — chat, expressions,
gauges, memory search and time travel all work over LAN HTTP. Only
push is disabled (the browser requires a secure context), and the
reach card says so honestly instead of showing a dead button.

## What's inside (the build so far)
...
- **Phase 8a — reach** (docs/PHASE8_REACH.md): the Observatory becomes
  an installable PWA, and the entity gains Web Push — VAPID, ECDH and
  RFC 8291 aes128gcm payload encryption implemented from the primes up
  in `anima/crypto/` (P-256, RFC 6979 ECDSA, AES-128-GCM — all pure
  stdlib, all pinned to NIST/RFC test vectors), a `notify` tool with a
  structural per-settle budget, and a phone-first mobile refit.
